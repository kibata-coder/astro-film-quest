    ...
    <div className="fixed inset-0 z-[100] bg-background" onMouseMove={handleMouseMove} onTouchStart={() => setShowControls(true)}>
      {isConnecting ? (
        ...
        <>
          ...
          {/* Server Switcher */}
          <div className={`absolute left-0 right-0 flex justify-center gap-4 transition-all duration-300 z-20 pointer-events-none ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} ${isTVShow && totalEpisodes ? 'bottom-24' : 'bottom-20'}`}>
          ...
          {isTVShow && totalEpisodes && (
            <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-background/90 to-transparent px-4 py-6 transition-all duration-300 pointer-events-none ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              <div className="flex items-center justify-between max-w-4xl mx-auto">
                <Button 
                  ...
                  className={`flex items-center gap-2 text-foreground hover:bg-foreground/10 disabled:opacity-30 ${showControls ? 'pointer-events-auto' : 'pointer-events-none'}`}
                >
                  <ChevronLeft className="w-5 h-5" /><span className="hidden sm:inline">Previous</span>
                </Button>
                
                <div className="text-center flex-1 px-4">
                  <p className="text-sm text-muted-foreground">Season {seasonNumber} • Episode {episodeNumber} of {totalEpisodes}</p>
                  <p className="text-foreground font-medium truncate max-w-xs sm:max-w-md mx-auto">{episodeName}</p>
                </div>
