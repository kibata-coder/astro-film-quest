...
import { getAnimeTVShows, TVShow } from '@/lib/tmdb';
...
const Anime = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) => getAnimeTVShows(pageParam),
    initialPageParam: 1,
  ...
  const shows: TVShow[] = data?.pages.flatMap((p) => p.results || []) || [];
    ...
    <Layout onSearch={setSearchQuery} searchQuery={searchQuery}>
      <main className="container mx-auto px-5 md:px-8 pt-28 pb-12">
        <h1 className="text-3xl font-bold mb-6 flex items-center gap-3">
          <Flame className="text-primary" /> Anime
        </h1>
